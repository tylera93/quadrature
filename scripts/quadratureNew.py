...
class QuadratureEstimator:
  ...
  def update(self, a_state, b_state, time):
    ...
    if offset is not None:
      # Update position
      self._position = self._position + (offset * self._ticks_per_revolution)
      ...
      if avg_time >= 1.0e-3:
        self._velocity = counter / avg_time * self._ticks_per_revolution
      else:
        self._velocity = 0
  ...
  @property
  def ticks_per_revoltion(self):
      return self._ticks_per_revoltion
